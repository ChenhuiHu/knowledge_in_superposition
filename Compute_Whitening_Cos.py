...
from compute import compute_k_C_k, compute_k
...
def cosine_similarity(model, tok, layer, k_i, k_j):
    """Calculate the cosine similarity between two vectors k_i and k_j."""
    ki_c_kj = compute_k_C_k(model, tok, layer, k_i, k_j)
    ki_c_ki = compute_k_C_k(model, tok, layer, k_i, k_i)
    kj_c_kj = compute_k_C_k(model, tok, layer, k_j, k_j)
    cosine_similarity = ki_c_kj / (torch.sqrt(ki_c_ki) * torch.sqrt(kj_c_kj))
    ...
    for layer in range(args.start_layer, args.end_layer):
        ...
        for i in tqdm(range(args.record_size), desc="Outer loop"):
            ...
            for j in tqdm(range(args.record_size), desc="Inner loop", leave=False):
                ...
                cos_matrix[i, j] = cosine_similarity(model, tokenizer, layer, ki, kj)
...
def save_cos_matrix(args, cos_matrix, layer):
    ...
    file_path = Path("data/whitening_cos_matrix") / filename
