...
from compute import compute_C_k, compute_k
...
def compute_p_matrix(args, model, tokenizer, dataset):
    """Compute the P Matrix for each layer."""
    context_templates = ['{}']
    print(f"Computing the PMatrix from layer {args.start_layer} to layer {args.end_layer}.")
    ...
    for layer in range(args.start_layer, args.end_layer):
        ...
        p_matrix = np.empty((args.record_size, args.record_size))
        ...
        for i in tqdm(range(args.record_size), desc="Outer loop"):
            ...
            for j in tqdm(range(args.record_size), desc="Inner loop", leave=False):
                ...
                p_matrix[i, j] = (torch.dot(C_ki, kj)) / (torch.dot(C_ki, ki))
        
        save_p_matrix(args, p_matrix, layer)

        
def save_p_matrix(args, p_matrix, layer):
    """Save the P Matrix and P Matrix without denominator."""
    filename = f"{args.model_name}/{args.knowledge_type}_sample_size_{args.record_size}/layer_{layer}_subject_last_p_matrix.npy"
    file_path = Path("data/p_matrix") / filename
    
    os.makedirs(file_path.parent, exist_ok=True)
    np.save(file_path, p_matrix)
    print(f"PMatrix saved to {file_path}")
...
def main():
    ...
    compute_p_matrix(args, model, tokenizer, dataset)
