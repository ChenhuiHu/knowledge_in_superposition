...
def compute_q_matrix(args, model, tokenizer, dataset):
    """Compute the Q Matrix for each layer."""
    context_templates = ['{}']
    print(f"Computing the QMatrix from layer {args.start_layer} to layer {args.end_layer}.")
    ...
    for layer in range(args.start_layer, args.end_layer):
        ...
        q_matrix = np.empty((args.record_size, args.record_size))
        ...
        for i in tqdm(range(args.record_size), desc="Outer loop"):
            ...
            for j in tqdm(range(args.record_size), desc="Inner loop", leave=False):
                ...
                q_matrix[i, j] = torch.dot(ki, kj)

        save_q_matrix(args, q_matrix, layer)


def save_q_matrix(args, q_matrix, layer):
    """Save the Q Matrix."""
    filename = f"{args.model_name}/{args.knowledge_type}_sample_size_{args.record_size}/layer_{layer}_subject_last_q_matrix.npy"
    file_path = Path("data/q_matrix") / filename

    os.makedirs(file_path.parent, exist_ok=True)
    np.save(file_path, q_matrix)
    print(f"QMatrix saved to {file_path}")
...
def main():
    ...
    compute_q_matrix(args, model, tokenizer, dataset)
